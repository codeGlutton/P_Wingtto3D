...
{
	GEN_STRUCT_REFLECTION_HEADER(TypeInfo)
...
public:
	...
	{
		return (_mTypeHash == other._mTypeHash) && IsA(other);
	}
...
namespace std {
	...
	{
		size_t operator()(const TypeInfo& key) const noexcept
		{
			std::hash<size_t> hasher;
			return hasher(key.GetHash());
		}
	};
}