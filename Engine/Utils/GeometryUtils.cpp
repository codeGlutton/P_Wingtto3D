...
{
	{
		...
		vertices[0].mPosition = Vec3(left, down, 0.f);
		vertices[0].mUV = { 0.f, 1.f };
		vertices[0].mTint = color;
		vertices[1].mPosition = Vec3(left, up, 0.f);
		vertices[1].mUV = { 0.f, 0.f };
		vertices[1].mTint = color;
		vertices[2].mPosition = Vec3(right, down, 0.f);
		vertices[2].mUV = { 1.f, 1.f };
		vertices[2].mTint = color;
		vertices[3].mPosition = Vec3(right, up, 0.f);
...
void PushQuad(OUT std::vector<UIVertexData>& vertices, OUT std::vector<uint32>& indices, float left, float right, float up, float down, Color color)
{
...

	{
		...
		vertices.push_back(UIVertexData{ Vec3(left, down, 0.f), { 0.f, 1.f }, color });
		vertices.push_back(UIVertexData{ Vec3(left, up, 0.f), { 0.f, 0.f }, color });
		vertices.push_back(UIVertexData{ Vec3(right, down, 0.f), { 1.f, 1.f }, color });
		vertices.push_back(UIVertexData{ Vec3(right, up, 0.f), { 1.f, 0.f }, color });
	}
...
void PushQuad(OUT std::vector<UIVertexData>& vertices, OUT std::vector<uint32>& indices, float left, float right, float up, float down, Vec2 uvStart, Vec2 uvSize, Color color)
{
	std::size_t preVBSize = vertices.size();

	{
		...
		vertices.push_back(UIVertexData{ Vec3(left, down, 0.f), { uvStart.x, uvStart.y + uvSize.y }, color });
		vertices.push_back(UIVertexData{ Vec3(left, up, 0.f), { uvStart.x, uvStart.y }, color });
		vertices.push_back(UIVertexData{ Vec3(right, down, 0.f), { uvStart.x + uvSize.x, uvStart.y + uvSize.y }, color });
		vertices.push_back(UIVertexData{ Vec3(right, up, 0.f), { uvStart.x + uvSize.x, uvStart.y }, color });
	}
